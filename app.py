...
# 2. Load Saved Model and Scaler Safely
@st.cache_resource
def load_assets():
    # GitHub ke mutabiq agar aapki file ka naam crop_gb_model_fixed.joblib ya pkl hai to wo likhein
...
# 3. Sidebar Navigation Panel (7 Pages)
...
# ----------------- PAGE 2: YIELD PREDICTOR ENGINE -----------------
    ...
    if model is None:
        st.error("⚠️ System Alert: Model file (`crop_gb_model_fixed.joblib`) is directory mein majood nahi hai. Kindly check karein.")
    else:
        ...
        # Preprocessing user selections to label encoded indices
        ...
        # Calculating Feature Engineering parameters in backend
...
elif page == "7. Student & Submission Info":  # <-- FIXED: Yahan naam bilkul sidebar jesa kr diya hai
